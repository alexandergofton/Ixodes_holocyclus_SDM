...
study_extent <- ext(140, 155, -44, -10)
...
occ_clean <- gbif_raw %>%
  ...
  filter(lon >= 140, lon <= 155, lat >= -44, lat <= -10) %>%
...
p_occ <- ggplot() +
  ...
  coord_sf(xlim = c(141, 155), ylim = c(-44, -10)) +
  labs(
...
png(file.path(figures_dir, "02_correlations.png"),
...
ggplot() +
  ...
  coord_sf(xlim = c(130, 155), ylim = c(-44, -10)) +
  labs(
...
p_points <- ggplot() +
  ...
  coord_sf(xlim = c(140, 155), ylim = c(-44, -10)) +
  labs(
...
# --- Random Forest: Mean Decrease Gini (built-in) ---
rf_imp <- importance(full_models$rf, type = 2)
rf_imp_df <- data.frame(
  Variable = rownames(rf_imp),
  Importance = rf_imp[, 1] / max(rf_imp[, 1]) * 100,
  Algorithm = "Random Forest"
)

# --- BRT: relative influence (built-in) ---
if (!is.null(full_models$brt)) {
  ...
  brt_imp_df <- data.frame(Variable = character(), Importance = numeric(),
                            Algorithm = character())
}

# --- MaxEnt: variance of marginal response curves ---
# Measure how much each variable drives predictions when varied across its range
occ_med_me <- apply(all_x[all_y == 1, ], 2, median, na.rm = TRUE)
me_var <- sapply(pred_cols, function(v) {
  vr <- seq(min(all_x[[v]], na.rm = TRUE), max(all_x[[v]], na.rm = TRUE),
            length.out = 100)
  pd <- as.data.frame(matrix(rep(occ_med_me, each = 100), nrow = 100,
                              dimnames = list(NULL, pred_cols)))
  pd[[v]] <- vr
  var(predict(full_models$maxent, pd, type = "cloglog"), na.rm = TRUE)
})
me_imp_df <- data.frame(
  Variable = pred_cols,
  Importance = me_var / max(me_var) * 100,
  Algorithm = "MaxEnt"
)

# --- GAM: chi-square statistic for each smooth term ---
gam_smry <- summary(full_models$gam)
gam_chi <- gam_smry$chi.sq
gam_imp_df <- data.frame(
  Variable = sub("^s\\(([^,)]+).*", "\\1", names(gam_chi)),
  Importance = as.numeric(gam_chi) / max(as.numeric(gam_chi)) * 100,
  Algorithm = "GAM"
)

# --- GLM: max |z-value| per variable (poly terms grouped back to variable name) ---
glm_coefs <- summary(full_models$glm)$coefficients
glm_coefs <- glm_coefs[rownames(glm_coefs) != "(Intercept)", , drop = FALSE]
var_names_glm <- sub("^poly\\(([^,]+),.*", "\\1", rownames(glm_coefs))
glm_z <- tapply(abs(glm_coefs[, "z value"]), var_names_glm, max)
glm_imp_df <- data.frame(
  Variable = names(glm_z),
  Importance = as.numeric(glm_z) / max(as.numeric(glm_z)) * 100,
  Algorithm = "GLM"
)

var_imp_all <- bind_rows(rf_imp_df, brt_imp_df, me_imp_df, gam_imp_df, glm_imp_df)
...
for (var in pred_cols) {
  ...
  resp_list <- list(
    MaxEnt = as.numeric(predict(full_models$maxent, pred_data, type = "cloglog")),
    RF     = predict(full_models$rf, pred_data, type = "prob")[, "1"],
    GAM    = as.numeric(predict(full_models$gam, pred_data, type = "response")),
    GLM    = as.numeric(predict(full_models$glm, pred_data, type = "response"))
  )
  if (!is.null(full_models$brt)) {
    resp_list$BRT <- predict(full_models$brt, pred_data,
                             n.trees = full_models$brt$gbm.call$best.trees,
                             type = "response")
  }

  response_df <- bind_rows(lapply(names(resp_list), function(nm) {
    data.frame(x = var_range, y = as.numeric(resp_list[[nm]]), Algorithm = nm)
  }))
  ...
  response_plots[[var]] <- ggplot(response_df, aes(x = x, y = y,
    ...
    geom_vline(xintercept = occ_range, linetype = "dashed", colour = "grey40",
               alpha = 0.7) +
...
p_response <- wrap_plots(response_plots, ncol = 3, guides = "collect") +
  plot_annotation(
    title = "Response curves across SDM algorithms",
    subtitle = "Grey dashed lines = observed range at occurrence points"
  ) &
...
## 9.1 Boyce-Weighted Ensemble

GAM is excluded due to low and highly variable Boyce index across spatial CV folds
(mean = 0.610, SD = 0.529), indicating unstable calibration. Remaining models are
weighted proportionally to their mean Boyce index from spatial cross-validation.

```{r ensemble-weights}
# GAM excluded: lowest Boyce index (0.610) with highest fold-to-fold variance (SD = 0.529)
boyce_scores <- cv_summary$Boyce_mean
names(boyce_scores) <- algorithms

# Zero out GAM and any negative/NA scores before normalising
boyce_scores["gam"] <- 0
boyce_scores[is.na(boyce_scores) | boyce_scores < 0] <- 0

weights <- boyce_scores / sum(boyce_scores, na.rm = TRUE)

cat("Ensemble weights (proportional to CV Boyce index, GAM excluded):\n")
for (i in seq_along(algo_names)) {
  cat(sprintf("  %s: %.3f (Boyce = %.3f)\n",
              algo_names[i], weights[i], cv_summary$Boyce_mean[i]))
}

# Build prediction list without GAM
pred_list <- list(maxent = pred_maxent, rf = pred_rf, glm = pred_glm)
weight_list <- weights[c("maxent", "rf", "glm")]
...
p_ensemble <- ggplot() +
  ...
  labs(
    title = expression(
      "Ensemble habitat suitability for" ~ italic("Ixodes holocyclus")
    ),
    subtitle = "Boyce-weighted mean of MaxEnt, RF, BRT, GLM (GAM excluded)",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggsave(
  file.path(figures_dir, "04_ensemble_suitability.png"),
  p_ensemble,
  width = 10,
  height = 10,
  dpi = 300
...
## 10.2 Interactive Ensemble Map

```{r leaflet-ensemble-map}
pal_leaf <- colorNumeric(
  palette = c("white", magma(256, direction = -1)),
  domain  = c(0, 1),
  na.color = "transparent"
)

# Convert terra SpatRaster to RasterLayer for leaflet compatibility
ens_rl <- raster::raster(ensemble_mean)

leaflet() %>%
  addTiles(group = "OpenStreetMap") %>%
  addRasterImage(ens_rl, colors = pal_leaf, opacity = 0.8,
  ...
  addLayersControl(
    ...
    overlayGroups = "Habitat Suitability",
    options = layersControlOptions(collapsed = FALSE)
  )
```

## 10.3 Binary Presence Map
...
## 10.4 Prediction Uncertainty Map
...
## 10.5 Individual Model Comparison
...
p_compare <- ggplot() +
  geom_raster(data = all_pred_df, aes(x = x, y = y, fill = suitability)) +
  ...
  labs(
    title = expression(
      "Individual model predictions for" ~ italic("Ixodes holocyclus")
    ),
    x = "Longitude",
    y = "Latitude"
  ) +
...
ggsave(
  file.path(figures_dir, "07_model_comparison.png"),
  p_compare,
  width = 14,
  height = 10,
  dpi = 300
...
maxent_df <- as.data.frame(pred_maxent, xy = TRUE, na.rm = TRUE)
names(maxent_df) <- c("x", "y", "suitability")

p_maxent <- ggplot() +
  geom_raster(data = maxent_df, aes(x = x, y = y, fill = suitability)) +
  geom_sf(data = aus, fill = NA, colour = "black", linewidth = 0.3) +
  ...
  coord_sf(xlim = c(141, 155), ylim = c(-44, -10)) +
  labs(
    title    = expression("MaxEnt habitat suitability for" ~ italic("Ixodes holocyclus")),
    x = "Longitude", y = "Latitude"
  ) +
  theme_minimal() +
  theme(legend.position = "right")
...
ggsave(file.path(figures_dir, "08_maxent_suitability.png"), p_maxent,
       width = 10, height = 10, dpi = 300)
```

```{r leaflet-maxent-map}
maxent_rl <- raster::raster(pred_maxent)

leaflet() %>%
  addTiles(group = "OpenStreetMap") %>%
  addRasterImage(maxent_rl, colors = pal_leaf, opacity = 0.8,
                 group = "MaxEnt Suitability") %>%
  addLegend(
    pal      = pal_leaf,
    values   = c(0, 1),
    title    = "MaxEnt<br>Suitability",
    position = "bottomright",
    opacity  = 0.9
  ) %>%
  addLayersControl(
    baseGroups    = "OpenStreetMap",
    overlayGroups = "MaxEnt Suitability",
    options = layersControlOptions(collapsed = FALSE)
  )
```

## 10.7 Save GeoTIFF Outputs
